...
- Hovering anywhere on the bar shows its color legend on the third row: color block + name + token count (narrow screens shorten the names).
...
- With usage data and the context bar enabled, the row defaults to occupancy, used capacity, remaining capacity (`free`), and per-type token counts, for example `5.8% · 57.9k/1m · free 942.1k · sys 1.4k …`. Hovering the bar temporarily shows its color legend; hovering another status field shows its details. Leaving restores the default summary. Content changes keep the same row height; minimal mode omits the default summary.
- Operation hints take precedence: running shows `esc to interrupt`, selecting shows `esc to return to input`; idle shows `? for shortcuts` only when `statusBar.shortcutHint` is enabled and help is closed. The default summary returns when the hint ends.
- When `statusBar.activity` is enabled and an activity summary is available, the default summary yields its space to that activity.
