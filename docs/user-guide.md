...
> 表内快捷键以 `Ctrl` 为准。macOS 的 `⌘` 别名只有在终端没有拦截、且通过扩展键盘协议传递时才可用（见 §5.4）；Terminal.app 用 Ctrl。
> 待办折叠默认按 **Control+Q（⌃Q）**。Ghostty 默认用 **Command+Q（⌘Q）** 退出应用，不会把它传给 dsh。
> 在 `/settings` 修改待办折叠快捷键后，面板和 `?` 帮助菜单会同步显示新绑定。
...
| `Ctrl+Q` | 折叠/展开待办列表，工作中也可使用；可在 `/settings` 改键 |
| `?` | 输入框为空时打开快捷键/命令帮助菜单 |
...
- 悬停整条时，第三行显示彩色图例：色块 + 名称 + token 数（窄屏自动改用短名）。
...
- 有上下文用量且进度条开启时，默认显示占用率、已用量、剩余容量 `free` 和各类 token 数，例如 `5.8% · 57.9k/1m · free 942.1k · sys 1.4k …`。悬停进度条时临时显示彩色图例，悬停其他状态字段时显示对应详情；移开后恢复默认明细。切换内容不增加行高；极简模式不显示默认明细。
- 操作提示优先于默认明细：运行中显示 `esc to interrupt`、选择中显示 `esc to return to input`；空闲的 `? for shortcuts` 仅在 `statusBar.shortcutHint` 开启且帮助未打开时显示。提示结束后恢复默认明细。
- 开启 `statusBar.activity` 且有活动摘要时，保留摘要，不用默认明细挤占其空间。
