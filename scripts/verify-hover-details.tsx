 ...
 *     文本是最右占比）；空闲时默认显示占比、已用量、剩余量和分类明细。
 *     悬停进度条显示彩色图例；悬停其他字段或出现操作提示时临时切换，
 *     结束后恢复，行高不变。
 *
...
try {
  ...
  check('I 未悬停时默认显示占比、已用、剩余及分类明细',
    await settled(() =>
      screenHas(barTerm, '19.5% · 12.5k/64k · free 51.5k')
      && screenHas(barTerm, 'sys 1.2k · pr 300 · ast 4k · th 5k · tl 2k')
      && !screenHas(barTerm, 'system 1.2k')))
  {
  ...
  // 悬停条最右（free 区）：彩色图例是「全部内容类型」，不只是 free。
  ...
  hover(barStdin, 1, 1)
  await sleep(100) // 固定窗:探针 移开后默认明细应恢复，不能轮询已成立的条件
  check('I 移开条恢复默认明细',
    screenHas(barTerm, '19.5% · 12.5k/64k · free 51.5k')
    && !screenHas(barTerm, 'thinking 5.0k'))
  ...
  // J：实际终端帧验证默认明细、悬停图例、临时内容和恢复；两种屏幕模式与宽度。
  for (const fullscreen of [true, false]) {
    for (const cols of [120, 80]) {
      ...
      const hasSummary = () => screenHas(rig.term, '19.5% · 12.5k/64k · free 51.5k')
      const hasLegend = () => screenHas(rig.term, 'free 52k')
      check(`${label} 默认完整明细可见`, await settled(() => hasSummary() && !hasLegend()))
      ...
      if (fullscreen) {
        hoverText(rig.stdin, rig.term, '19.5%')
        check(`${label} 悬停进度条改显示彩色图例`,
          await settled(() => hasLegend() && !hasSummary()))
        hover(rig.stdin, 1, 1)
        check(`${label} 移开进度条恢复完整明细`, await settled(hasSummary))
        hoverText(rig.stdin, rig.term, 'TM')
        check(`${label} model 详情临时替换默认明细`,
          await settled(() => screenHas(rig.term, 'provider test-provider') && !hasSummary()))
        check(`${label} 悬停不增加行高`, remainsSameHeight())
        hover(rig.stdin, 1, 1)
        check(`${label} 移开后恢复完整明细`, await settled(hasSummary))
      }

      mounted.rerender(draw({ channel: { ...barStub, working: true } as never }))
      check(`${label} 执行中操作提示优先`,
        await settled(() => screenHas(rig.term, 'esc 中断') && !hasSummary()))
      mounted.rerender(draw({ selectionActive: true }))
      check(`${label} 文本选择提示优先`,
        await settled(() => screenHas(rig.term, 'esc 返回输入') && !hasSummary()))
      mounted.rerender(draw())
      check(`${label} 操作结束恢复完整明细且高度不变`,
        await settled(() => hasSummary() && remainsSameHeight()))
      ...
      check(`${label} 用户开启的空闲提示仍优先`,
        await settled(() => screenHas(rig.term, '? 查看快捷键') && !hasSummary()))
      mounted.rerender(draw({ channel: withHints as never, helpOpen: true }))
      check(`${label} 快捷键提示隐藏后恢复完整明细`, await settled(hasSummary))
      ...
      check(`${label} 活动摘要不被默认明细挤占`,
        await settled(() => screenHas(rig.term, 'Finished fixture task') && !hasSummary()))
      mounted.rerender(draw({ channel: { ...barStub, contextBarEnabled: false } as never }))
      check(`${label} 关闭进度条后不残留默认明细`,
        await settled(() => !hasSummary() && !screenHas(rig.term, 'Finished fixture task')))
      mounted.rerender(draw())
      check(`${label} 重新开启进度条恢复完整明细`, await settled(hasSummary))
      mounted.rerender(draw({ channel: { ...barStub, lastUsage: undefined } as never }))
      check(`${label} 新会话无用量时不显示虚构明细`, await settled(() => !hasSummary()))
      mounted.rerender(draw())
      check(`${label} 获取用量后恢复完整明细`, await settled(hasSummary))
      mounted.rerender(draw({ channel: { ...barStub, minimal: true } as never }))
      check(`${label} 极简模式不新增默认明细`, await settled(() => !hasSummary()))
      mounted.unmount()
