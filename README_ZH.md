...
- **待办快捷键提示与配置一致。** 面板与帮助菜单使用实际配置的按键，详见[快捷键说明](docs/user-guide.md#2-快捷键速查)。
- **紧凑输入区。** 输入框上方只在有通知时占一行，通知消失后收回空行。
...
- **空闲时显示上下文用量。** 底部没有提示或活动摘要时显示占用率、已用量、剩余量和分类 token 数；悬停进度条可查看彩色图例。详见[状态栏说明](docs/user-guide.md#52-底部状态栏输入框下方三行)。
