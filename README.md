...
- **Compact input area:** removes the blank row above the input box; a notification uses that row only while visible.
...
- **Context details while idle:** the footer shows usage percentage, used and free tokens, and per-type counts whenever no hint or activity summary is displayed. Hovering the context bar still shows its color legend. See the [status bar guide](docs/user-guide.en.md#52-bottom-status-bar-three-rows-under-the-input).
